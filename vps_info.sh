...
get_congestion_control() {
    ...
    # 方案2: 方案1失败则尝试从当前连接堆栈获取 (LXC环境)
    if [ -z "$tcp_cc" ] || [ "$tcp_cc" == "未知" ]; then
        tcp_cc=$(ss -itn 2>/dev/null | grep -oP '(?<= )(cubic|bbr|reno|hybla|vegas)(?= )' | head -n 1)
    fi
    
    if [ -z "$tcp_cc" ]; then
        echo -e "${YELLOW}未知 (受限环境)${PLAIN}"
    elif [ "$tcp_cc" == "bbr" ]; then
        echo -e "${GREEN}BBR (加速中)${PLAIN}"
    else
        echo -e "${CYAN}${tcp_cc}${PLAIN}"
    fi
}
...
cc_status=$(get_congestion_control)
